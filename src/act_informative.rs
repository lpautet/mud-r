...
fn look_at_char(game: &mut Game, db: &DB, i_id: DepotId, chid: DepotId) {
    let i = db.ch(i_id);
    let ch = db.ch(chid);
    let mut found;
...
fn list_one_char(game: &mut Game, db: &DB, i_id: DepotId, chid: DepotId) {
...
pub fn do_examine(
    ...
    if tmp_object_id.is_some() {
        ...
        let tmp_object = db.obj(tmp_object_id);
