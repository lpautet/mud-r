...
use crate::structs::{
    CharData, ObjFileElem, RoomFlags, RoomRnum, RoomVnum, LVL_GRGOD, LVL_IMMORT, NOWHERE,
    NUM_OF_DIRS,
};
...
fn house_restore_weight(objs: &mut Depot<ObjData>, oids: Vec<DepotId>) {
    for oid in oids {
        for coid in objs.get(oid).contains.clone() {
...
pub fn house_crashsave(db: &mut DB, objs: &mut Depot<ObjData>, vnum: RoomVnum) {
    ...
    house_restore_weight(objs, db.world[rnum as usize].contents.clone());
...
fn find_house(db: &DB, vnum: RoomVnum) -> Option<usize> {
    db.house_control.iter().position(|hc| hc.vnum == vnum)
}
...
-------  ------  ----------  ------  ------------ ----------\r\n",
    ...
    for (i, house_control) in house_control.iter().enumerate().take(db.num_of_houses) {
        ...
        let last_pay = if house_control.last_payment != 0 {
            ctime(house_control.last_payment)
        } else {
            "None".to_string()
        };
...
fn hcontrol_destroy_house(
    ...
    if res {
        ...
        db.remove_room_flags_bit(
            real_house,
            RoomFlags::HOUSE | RoomFlags::PRIVATE | RoomFlags::HOUSE_CRASH,
        );
    }
...
pub fn house_save_all(db: &mut DB, objs: &mut Depot<ObjData>) {
    for i in 0..db.num_of_houses {
        ...
        if real_house != NOWHERE && db.room_flagged(real_house, RoomFlags::HOUSE_CRASH) {
