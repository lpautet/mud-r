...
use crate::db::{fread_string, LoadType, DB};
use crate::handler::{
    fname, get_number, get_obj_in_list_num, get_obj_in_list_vis, isname, obj_from_char, obj_to_char,
};
use crate::interpreter::{cmd_is, find_command, is_number, one_argument, SCMD_EMOTE};
use crate::modify::page_string;
use crate::structs::{
    AffectFlags, CharData, ExtraFlags, ItemType, MeRef, MobRnum, MobVnum, ObjData, ObjVnum,
    RoomRnum, RoomVnum, Special, LVL_GOD, MAX_OBJ_AFFECT, NOBODY, NOTHING, NOWHERE,
};
...
fn is_ok_char(
        ...
        || ch.is_neutral() && db.shop_index[shop_nr].notrade_neutral()
    {
        ...
        do_tell(
            ...
            keeper_id,
            &buf,
            CMD_TELL.load(Ordering::Relaxed),
            0,
        );
        return false;
    }
    ...
    while !expr[ptr..].is_empty() {
        if &expr[ptr..ptr] == " " {
            ...
            if let Some(temp) = temp {
                if temp != OPER_OPEN_PAREN as usize {
                    while top(&ops) > temp as i32 {
                        evaluate_operation(&mut ops, &mut vals);
                    }
                }

                if temp == OPER_CLOSE_PAREN as usize {
                ...
                let mut findex = 0;
                for (eindex, exbits) in EXTRA_BITS.iter().enumerate() {
                    if name == *exbits {
                        push(
                            &mut vals,
                            if obj.obj_flagged(ExtraFlags::from_bits_truncate(1 << eindex)) {
                                1
                            } else {
                                0
                            },
...
fn transaction_amt(arg: &mut String) -> i32 {
    ...
    if !buywhat.is_empty() && !buf.is_empty() && is_number(&buf) {
        ...
        return buf
            .parse::<i32>()
            .expect("should be an int (is_number said it is)");
    }
    1
}
...
fn times_message(obj: Option<&ObjData>, name: &str, num: i32) -> String {
    let mut buf = String::new();
    if let Some(obj) = obj {
        buf.push_str(obj.short_description.as_ref());
    } else {
        let pos = name.find('.');
        let ptr = if pos.is_none() { name } else { &name[1..] };
...
fn get_slide_obj_vis(
    ...
    for i in list {
        ...
        if isname(&tmpname, objs.get(*i).name.as_ref())
            && can_see_obj(&game.descriptors, chars, db, ch, objs.get(*i))
            && last_match.is_none_or(|lm| !same_obj(objs.get(lm), objs.get(*i)))
        {
            if j == number {
                return Some(*i);
            }
...
fn get_hash_obj_vis(
    ...
    if let Ok(num) = name.parse::<i32>() {
        qindex = num;
    } else if let Ok(num) = name[1..].parse::<i32>() {
        qindex = num;
    } else {
        return None;
    }
    ...
    for l in list.iter() {
        if can_see_obj(&game.descriptors, chars, db, ch, objs.get(*l))
            && objs.get(*l).get_obj_cost() > 0
            && last_obj.is_some_and(|last_obj| !same_obj(objs.get(last_obj), objs.get(*l)))
        {
            let res = {
                qindex -= 1;
                qindex == 0
...
fn get_purchase_obj(
    ...
    loop {
        ...
        if objs.get(oidv).get_obj_cost() <= 0 {
            db.extract_obj(chars, objs, oidv);
            oid = None;
        }
        if oid.is_some() {
            break;
...
fn buy_price(
    ...
    (objs.get(oid).get_obj_cost() as f32
 ...
 */
fn sell_price(obj: &ObjData, shop: &ShopData, keeper: &CharData, seller: &CharData) -> i32 {
...
fn shopping_buy(
    ...
    let mut oido = get_purchase_obj(
        game, chars, db, texts, objs, chid, &arg, keeper_id, shop_nr, true,
    );
    let Some(mut oid) = oido else {
        return;
    };
    let ch = chars.get(chid);
    if buy_price(chars, db, objs, oid, shop_nr, keeper_id, chid) > ch.get_gold() && !ch.is_god() {
    ...
    if ch.is_carrying_n() + 1 > ch.can_carry_n() as u8 {
        send_to_char(
            ...
            format!(
                ...
                fname(objs.get(oid).name.as_ref())
            )
            .as_str(),
        );
        return;
    }
    if ch.is_carrying_w() + objs.get(oid).get_obj_weight() > ch.can_carry_w() as i32 {
        send_to_char(
            &mut game.descriptors,
            ch,
            format!(
    ...
    let mut bought = 0;
    let mut goldamt = 0;
    let mut last_oid = oid;
    while {
        let ch = chars.get(chid);
        oido.is_some()
            && (ch.get_gold() >= buy_price(chars, db, objs, oid, shop_nr, keeper_id, chid)
                || ch.is_god())
            && ch.is_carrying_n() < ch.can_carry_n() as u8
            && bought < buynum
            && ch.is_carrying_w() + objs.get(oid).get_obj_weight() <= ch.can_carry_w() as i32
    } {
        bought += 1;

        /* Test if producing shop ! */
        if shop_producing(db, objs, oid, shop_nr) {
            oido = db.read_object(objs, objs.get(oid).get_obj_rnum(), LoadType::Real);
            oid = oido.expect("Failed to read object");
        } else {
        ...
        let charged = buy_price(chars, db, objs, oid, shop_nr, keeper_id, chid);
        goldamt += charged;
        let ch = chars.get(chid);
        if !ch.is_god() {
        ...
        last_oid = oid;
        oido = get_purchase_obj(
            game, chars, db, texts, objs, chid, &arg, keeper_id, shop_nr, false,
        );
        if oido.is_some_and(|id| {
            oid = id;
            !same_obj(objs.get(oid), objs.get(last_oid))
        }) {
            break;
        }
    }
    ...
    if bought < buynum {
        let ch = chars.get(chid);
        if oido.is_none_or(|oid| !same_obj(objs.get(last_oid), objs.get(oid))) {
            buf = format!("{} I only have {} to sell you.", ch.get_name(), bought);
        } else if ch.get_gold() < buy_price(chars, db, objs, oid, shop_nr, keeper_id, chid) {
    ...
    let obj = oido.map(|id| objs.get(id));
    act(
        &mut game.descriptors,
        chars,
        db,
...
fn get_selling_obj(
    ...
    let ch = chars.get(chid);
    let Some(obj) = get_obj_in_list_vis(
        &game.descriptors,
        chars,
        db,
        objs,
    ...
    let buf = match result {
        ...
        OBJECT_NOTOK => db.shop_index[shop_nr]
            .do_not_buy
            .replace("%s", ch.get_name()),
        OBJECT_DEAD => {
            format!("{} {}", ch.get_name(), MSG_NO_USED_WANDSTAFF)
        }
        _ => {
            error!(
            ...
            format!("{} An error has occurred.", ch.get_name())
        }
    };
    do_tell(
        game,
...
fn slide_obj(
    ...
    */
    ...
    let keeper = chars.get_mut(keeper_id);
    if let Some(idx) = idx {
        keeper.carrying.insert(idx, obj_id);
    } else {
...
fn sort_keeper_objs(
    ...
    while {
        let keeper = chars.get(keeper_id);
        db.shop_index[shop_nr].lastsort < keeper.is_carrying_n() as i32
    } {
        let keeper = chars.get(keeper_id);
...
fn shopping_sell(
    ...
    let mut name = String::new();
    one_argument(&arg, &mut name);
    let Some(oid) = get_selling_obj(
        game, chars, db, texts, objs, chid, &name, keeper_id, shop_nr, 1,
    ) else {
        return;
    };
    let ch = chars.get(chid);
    ...
    while let Some(oidv) = oid {
        let keeper = chars.get(keeper_id);
        let ch = chars.get(chid);
        if keeper.get_gold() + db.shop_index[shop_nr].bank_account
            >= sell_price(objs.get(oidv), &db.shop_index[shop_nr], keeper, ch)
            && sold < sellnum
        {
            let ch = chars.get(chid);
            let keeper = chars.get(keeper_id);
            let charged = sell_price(objs.get(oidv), &db.shop_index[shop_nr], keeper, ch);
            ...
            sold += 1;
            obj_from_char(chars, objs.get_mut(oidv));
            slide_obj(game, chars, db, objs, oidv, keeper_id, shop_nr); /* Seems we don't use return value. */
            oid = get_selling_obj(
                game, chars, db, texts, objs, chid, &name, keeper_id, shop_nr, 0,
            );
        }
    }
    let ch = chars.get(chid);
...
pub fn shopping_list(
        ...
        for &oid in &keeper.carrying {
                ...
                && objs.get(oid).get_obj_cost() > 0
            {
                match last_oid {
                    ...
                    Some(last_oidv) if same_obj(objs.get(last_oidv), objs.get(oid)) => {
                        cnt += 1;
                    }
                    Some(last_oidv) => {
                        lindex += 1;
                        if name.is_empty() || isname(&name, objs.get(last_oidv).name.as_ref()) {
                            buf.push_str(&list_object(
                                db, chars, last_oidv, objs, cnt, lindex, shop_nr, keeper_id, chid,
                            ));
                            found = true;
                        }
                        cnt = 1;
    ...
    match last_oid {
        ...
        Some(last_oid) => {
            if name.is_empty() || isname(&name, objs.get(last_oid).name.as_ref()) {
                /* show last obj */
                buf.push_str(&list_object(
                    db, chars, last_oid, objs, cnt, lindex, shop_nr, keeper_id, chid,
                ));
            }
            let ch = chars.get(chid);
            let d_id = ch.desc.expect("Character has no descriptor");
...
pub fn shop_keeper(
    ...
    if let Some(func) = db.shop_index[shop_nr].func {
    ...
    let res = {
        ...
        !ok_shop_room(&db.shop_index[shop_nr], room)
    };
    if res {
        return false;
    }
    ...
    if cmd_is(cmd, "buy") {
        shopping_buy(
            game, chars, db, texts, objs, argument, chid, keeper_id, shop_nr,
        );
        return true;
...
pub fn ok_damage_shopkeeper(
    ...
    if !db.is_mob(victim)
        || db.mob_index[victim.get_mob_rnum() as usize]
            .func
            .is_some_and(|func| func as usize != shop_keeper as usize)
    {
        return true;
    }
...
pub fn boot_the_shops(db: &mut DB, shop_f: File, filename: &str, _rec_count: i32) {
    ...
    let regex = Regex::new(r"^#(-?\+?\d{1,9})").expect("Regex compilation failed");
    while !done {
        let buf = fread_string(&mut reader, &buf2);
        if buf.starts_with('#') {
            /* New shop */

            let f = regex.captures(&buf).expect("Regex capture failed");
            let mut temp = f[1].parse::<i32>().expect("should be an int");
            ...
            for item in list.iter().take(temp as usize) {
                shop.type_.push({
                    ShopBuyData {
                        type_: item.type_,
            ...
            for item in list.iter().take(temp as usize) {
                shop.in_room.push(item.type_ as RoomVnum);
            }
...
pub fn assign_the_shopkeepers(db: &mut DB) {
    CMD_SAY.store(
        find_command("say").expect("Command 'say' not found"),
        Ordering::Relaxed,
    );
    CMD_TELL.store(
        find_command("tell").expect("Command 'tell' not found"),
        Ordering::Relaxed,
    );
    CMD_TELL.store(
        find_command("emote").expect("Command 'emote' not found"),
        Ordering::Relaxed,
    );
    CMD_SLAP.store(
        find_command("slap").expect("Command 'slap' not found"),
        Ordering::Relaxed,
    );
    CMD_PUKE.store(
        find_command("puke").expect("Command 'puke' not found"),
        Ordering::Relaxed,
    );
    ...
    for shop in db.shop_index.iter_mut() {
        ...
        if db.mob_index[shop.keeper as usize]
            .func
            .is_some_and(|func| func as usize != shop_keeper as usize)
        {
            shop.func = db.mob_index[shop.keeper as usize].func;
        }
    }
}
...
---------------------------------------------------------\r\n";
    ...
    let d_id = ch.desc.expect("should have a descriptor");
    page_string(&mut game.descriptors, chars, d_id, &buf, true);
}

fn list_detailed_shop(
    game: &mut Game,
    chars: &Depot<CharData>,
    db: &DB,
    chid: DepotId,
    ...
    for sindex in 0..db.shop_index[shop_nr as usize].in_room.len() {
        ...
        let buf1 = if res {
            ...
            format!(
                "<UNKNOWN> (#{})",
                db.shop_index[shop_nr as usize].in_room[sindex]
            )
        };
    ...
    if db.shop_index[shop_nr as usize].keeper != NOBODY {
        ...
        if let Some(k) = db.get_char_num(chars, db.shop_index[shop_nr as usize].keeper) {
            send_to_char(
                &mut game.descriptors,
                ch,
                format!(
    ...
    while db.shop_index[shop_nr as usize].shop_product(sindex) != NOTHING {
        ...
        let nbuf = format!(
            "{} (#{})",
            db.obj_proto[db.shop_index[shop_nr as usize].shop_product(sindex) as usize]
                .short_description,
    ...
    while db.shop_index[shop_nr as usize].type_[sindex as usize].type_ != NOTHING as i32 {
        ...
        let buf1 = format!(
            ...
            if !db.shop_index[shop_nr as usize].type_[sindex as usize]
                .keywords
                .is_empty()
            {
    ...
    send_to_char(
        ...
        format!(
            ...
            db.shop_index[shop_nr as usize].profit_sell,
            db.shop_index[shop_nr as usize].profit_buy,
            db.shop_index[shop_nr as usize].open1,
            db.shop_index[shop_nr as usize].close1,
            db.shop_index[shop_nr as usize].open2,
            db.shop_index[shop_nr as usize].close2
        )
        .as_str(),
    );
    ...
    let mut buf1 = String::new();
    sprintbit(
        db.shop_index[shop_nr as usize].bitvector as i64,
        &SHOP_BITS,
        &mut buf1,
    );
    send_to_char(
...
pub fn show_shops(
    ...
    if arg.is_empty() {
        ...
        if let Some(shop_nr) = shop_nr {
            list_detailed_shop(game, chars, db, chid, shop_nr);
        } else {
